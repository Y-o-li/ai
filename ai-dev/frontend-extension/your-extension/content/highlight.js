    ...
    function processBatchWithTimeSlicing(deadline) {
        ...
        while (currentIndex < nodesToProcess.length && 
            ...
            if (result.isInciting) {
                ...
                detectedCount++;
                // 更新检测统计
                console.log('[Highlight] 检测到煽动性内容，准备更新统计');
                    ...
                    if (entry.isIntersecting) {
                        ...
                        if (text.trim().length >= detector.config.minTextLength) {
                            ...
                            if (result.isInciting) {
                                ...
                                // 更新检测统计
                                console.log('[Highlight] 懒加载检测到煽动性内容，准备更新统计');
                                updateHighlightStats();
                            }
                        }
...
function detectNode(node) {
    ...
    if (result.isInciting) {
        detector.highlight(node, result);
        // 更新检测统计
        console.log('[Highlight] detectNode 检测到煽动性内容，准备更新统计');
        updateHighlightStats();
    }
}
...
/**
 * 更新高亮检测统计
 * 通过 postMessage 通知 content-script.js 来更新统计
 */
function updateHighlightStats() {
    console.log('[Highlight] updateHighlightStats 被调用，发送消息给 content-script');
    try {
        // 通过 postMessage 通知 content-script.js 更新统计
        window.postMessage({
            type: 'YANZHI_YOULI_UPDATE_HIGHLIGHT_STATS'
        }, '*');
    } catch (error) {
        console.error('[Highlight] 发送统计更新消息失败:', error);
    }
}

// 导出模块
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { SensitiveWordDetector, scanAndHighlight, detectNode, updateHighlightStats };
}
