...
function applyThemeToPage(theme) {
    ...
    window.addEventListener('message', (event) => {
        ...
        // 处理高亮统计更新请求
        if (event.data && event.data.type === 'YANZHI_YOULI_UPDATE_HIGHLIGHT_STATS') {
            console.log('[Content Script] 收到高亮统计更新请求');
    ...
    function scanElement(element) {
        ...
        nodes.forEach(textNode => {
            ...
            if (result.isInciting) {
                ...
                // 更新检测统计
                console.log('[Content Script] 检测到煽动性内容，调用 updateHighlightCount');
                updateHighlightCount();
            }
        });
    }
    ...
    async function updateHighlightCount() {
        try {
            const result = await chrome.storage.local.get(['highlightCount']);
            const currentCount = result.highlightCount || 0;
            await chrome.storage.local.set({ highlightCount: currentCount + 1 });
            console.log('[Content Script] 已更新检测统计为:', currentCount + 1);
        } catch (error) {
            console.error('[Content Script] 更新检测统计失败:', error);
        }
    }
