...
async function updateProcessCount() {
  try {
    const result = await chrome.storage.local.get(['processCount']);
    const currentCount = result.processCount || 0;
    await chrome.storage.local.set({ processCount: currentCount + 1 });
    console.log('[Service Worker] 已更新处理统计:', currentCount + 1);
  } catch (error) {
    console.error('[Service Worker] 更新处理统计失败:', error);
  }
}
